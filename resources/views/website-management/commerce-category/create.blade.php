    ...
    <div class="col-lg-6 col-12">
        <div class="box">
            <div class="box-header with-border">
                <h4 class="box-title">Create A New Commerce Category</h4>
                {{-- <h4 class="box-title"></h4> --}}
            </div>

            <form class="form" action="{{ route('website-management.commerce-category.store') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="box-body">
                    <div class="form-group @error('name') error @enderror">
                        <label class="form-label">Commerce Category <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" placeholder="Finish Product" name="name" id="name" value="{{ old('name') }}">
                        @error('name')
                    ...
                    <div class="form-group @error('slug') error @enderror">
                        ...
                        <input type="text" class="form-control" placeholder="finish-product" name="slug" id="slug" value="{{ old('slug') }}">
                        @error('slug')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
